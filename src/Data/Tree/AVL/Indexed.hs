...
insertWith :: Ord k => (v -> v -> v) -> k -> v -> Tree k v h -> Inserted k v h
insertWith _ v vc Leaf = Incr (Node v vc O Leaf Leaf)
insertWith f v vc (Node k kc bl tl tr) = case compare v k of
  LT -> case insertWith f v vc tl of
  ...
  GT -> case insertWith f v vc tr of
...
lookup k (Node v vc _ tl tr) = case compare k v of
  LT -> lookup k tl
  EQ -> Just vc
  GT -> lookup k tr
...
uncons :: k -> v -> Balance lh rh h -> Tree k v lh -> Tree k v rh -> (k , v, Deleted k v (S h))
uncons (k' :: k) (v' :: v) bl' tl' tr' = go k' v' bl' tl' tr' id
  where
    go :: forall x lh rh h. k -> v -> Balance lh rh h -> Tree k v lh -> Tree k v rh -> (Deleted k v (S h) -> x) ->  (k , v, x)
    go k v O Leaf tr c = (k , v, c (Decr tr))
    go k v O (Node kl vl bl tll trl) tr c = go kl vl bl tll trl $ \case
        Decr ntl -> c (Hold (Node k v R ntl tr))
        Hold ntl -> c (Hold (Node k v O ntl tr))
    go k v R Leaf tr c = (k , v, c (Decr tr))
    go k v R (Node kl vl bl tll trl) tr c = go kl vl bl tll trl $ \case
        Decr ntl -> c (deleted (rotl k v ntl tr))
        Hold ntl -> c (Hold (Node k v R ntl tr))
    go k v L (Node kl vl bl tll trl) tr c = go kl vl bl tll trl $ \case
...
delete k (Node k' v b tl tr) = case compare k k' of
  ...
  EQ -> case tr of
    Leaf -> case b of
      L -> Decr tl
      O -> Decr tl
    Node kr vr br tlr trr -> case b of
      L -> case uncons kr vr br tlr trr of
        (k'', v', Decr tr') -> deleted (rotr k'' v' tl tr')
        (k'', v', Hold tr') -> Hold (Node k'' v' L tl tr')
      O -> case uncons kr vr br tlr trr of
        (k'', v', Decr tr') -> Hold (Node k'' v' L tl tr')
        (k'', v', Hold tr') -> Hold (Node k'' v' O tl tr')
      R -> case uncons kr vr br tlr trr of
        (k'', v', Decr tr') -> Decr (Node k'' v' O tl tr')
        (k'', v', Hold tr') -> Hold (Node k'' v' R tl tr')
